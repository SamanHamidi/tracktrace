...
class ForcastUtility:
    ...
    def _get_coordinates(self):
        if cached_coordinates := cache.get(key=f'{self.zip_code}:{self.country_code}', default=False):
        ...
        if resp.ok:
            ...
            cache.set(key=f'{self.zip_code}:{self.country_code}', value=coordinates, timeout=None)
            return coordinates
